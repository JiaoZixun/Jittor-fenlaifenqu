...
parser.add_argument("--pretrained", type=str, default='/root/autodl-tmp/PASS-jittor/checkpoint.pth.tar', help="the model checkpoint")
parser.add_argument("--data_path", type=str, default='/root/autodl-tmp/ImageNetS50', help="path to data")
parser.add_argument("--dump_path", type=str, default='.', help="path to save clustering results")
...
def getEmb(dataloader, model, size):
    ...
    with jt.no_grad():
        for idx, path, inputs, target in tqdm(dataloader):
            ...
            if start_idx == 0:
                ...
                embeddings = jt.zeros((size, emb.shape[1]), dtype='float32')
    ...
    return indexes.numpy(), targets.numpy(), embeddings.numpy(), paths
